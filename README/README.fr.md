...
**Un fork de [VRCX](https://github.com/vrcx-team/VRCX) avec un thème sombre moderne et des fonctionnalités améliorées.**
...
**Note:** Cette traduction peut ne pas être à jour. Consultez la [version anglaise](/README.md) pour les informations les plus récentes, notamment sur la migration depuis VRCX.

VRCXM est un fork de VRCX, une application assistante/complémentaire pour VRChat qui fournit des informations et vous aide à accomplir diverses tâches liées à VRChat de manière plus pratique que de compter uniquement sur le client VRChat classique (desktop ou VR) ou le site web seul. Ce fork propose une refonte complète avec un thème sombre moderne et des fonctionnalités supplémentaires tout en conservant une fonctionnalité 1:1 avec le VRCX original.

# Pour Commencer

<div align="center">

**Note:** VRCXM est un fork et doit être compilé à partir des sources. Voir [Compilation à partir des sources](#compilation-à-partir-des-sources) ci-dessous.

Le programme d'installation VRCX original (`VRCX_Setup.exe`) est disponible [ici](https://github.com/vrcx-team/VRCX/releases/latest).
...
VRCXM doit être compilé à partir des sources. Consultez le guide original de VRCX [Compilation à partir des sources](https://github.com/vrcx-team/VRCX/wiki/Building-from-source) pour les instructions. Le processus de compilation est identique à VRCX.

## Migration depuis VRCX

VRCXM utilise un dossier de données séparé (`VRCXM`) pour éviter les conflits avec le VRCX original. Pour migrer vos données VRCX existantes (amis, historique du flux, favoris, etc.) vers VRCXM, consultez la [version anglaise du README](/README.md#migrating-from-vrcx) pour les instructions détaillées.

## Différences par rapport à VRCX

- **Thème Sombre Moderne** : Refonte visuelle complète avec une esthétique sombre moderne
- **Filtres de Flux Avancés** : Filtrage par plage de dates et filtres de tags/types améliorés pour le flux d'activité
- **Masquer des Amis du Flux** : Option pour masquer des amis spécifiques du flux d'activité tout en continuant à suivre leurs données
- **Tableau de Bord Analytique** : Page d'analyses complète avec :
  - Analyses d'Activité : Répartition du temps passé par monde/avatar
  - Statistiques Sociales : Classement des amis les plus actifs
  - Analyses Monde & Avatar : Mondes les plus visités et statistiques d'utilisation d'avatars
- **Paramètres de Performance** : Taille maximale de table configurable pour les requêtes de flux et de journal de jeu
- **Données d'Application Séparées** : Utilise le dossier `VRCXM` au lieu de `VRCX`
- **Toutes les fonctionnalités VRCX originales préservées** : Parité fonctionnelle 1:1 avec l'original

## Mainteneur

VRCXM est maintenu par [naiolune](https://github.com/naiolune).

Le VRCX original est développé par [pypy](https://github.com/pypy-vrc) et [Natsumi](https://github.com/Natsumi-sama).

## Ressources VRCX Originales
...
## VRCXM enfreint-il les CGU de VRChat ?

**Non.**

VRCXM (comme VRCX) est un outil externe qui utilise l'API VRChat pour fournir les fonctionnalités qu'il propose.
