...
export function request(endpoint, options) {
    ...
    let req;
    // Ensure endpointDomain is set before creating URL
    if (!AppDebug.endpointDomain) {
        AppDebug.endpointDomain = AppDebug.endpointDomainVrchat || 'https://api.vrchat.cloud/api/1';
    }
    const init = {
    ...
    if (!init.url || typeof init.url !== 'string') {
        throw new Error(`Invalid URL for endpoint "${endpoint}": ${init.url}`);
    }
    const { params } = init;
    if (init.method === 'GET') {
        ...
        .then(({ data, status }) => {
            ...
            if (status === 401) {
                if (data.error?.message === '"Missing Credentials"') {
                    // Try auto-login, but don't throw if it fails (user will need to log in manually)
                    authStore.handleAutoLogin().catch(() => {
                        // Auto-login failed - user needs to log in manually, this is expected
                    });
                    $throw(
                        401,
