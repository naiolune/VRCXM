        ...
        for (var tableName of tables) {
            if (tableName && typeof tableName === 'string' && tableName.trim() !== '' && !tableName.includes('undefined')) {
                try {
                    ...
                    e = e.toString();
                    if (e.indexOf('duplicate column name') === -1 && e.indexOf('no such table') === -1) {
                        console.warn(`Error updating table ${tableName} for group names:`, e);
                    }
                }
            }
        }
        ...
        for (var tableName of tables) {
            if (tableName && typeof tableName === 'string') {
                try {
                    ...
                    e = e.toString();
                    if (e.indexOf('duplicate column name') === -1 && e.indexOf('no such table') === -1) {
                        console.warn(`Error adding friend_number to table ${tableName}:`, e);
                    }
                }
            }
        }
        ...
        for (var tableName of tables) {
            if (tableName && typeof tableName === 'string') {
                try {
                    ...
                    e = e.toString();
                    if (e.indexOf('duplicate column name') === -1 && e.indexOf('no such table') === -1) {
                        console.warn(`Error updating table ${tableName} for avatar history:`, e);
                    }
                }
            }
        }
