...
<script setup>
            ...
            AppApi.FindScreenshotsBySearch(D.search, searchType)
                .then((json) => {
                    ...
                    if (!json || json === 'undefined' || json === 'null' || typeof json !== 'string') {
                        console.error('Invalid search results JSON:', json);
                        D.metadata = {};
                        D.metadata.error = 'Invalid search results.';
                        D.searchIndex = null;
                    ...
                    try {
                        ...
                        console.error('Error parsing search results JSON:', e, 'Input:', json);
                        D.metadata = {};
                        D.metadata.error = `Error parsing search results: ${e.message}`;
                        D.searchIndex = null;
                        D.searchResults = null;
                        D.loading = false;
                        return;
    ...
    async function displayScreenshotMetadata(json, needsCarouselFiles = true) {
        ...
        if (!json || json === 'undefined' || json === 'null' || typeof json !== 'string') {
            D.metadata = {};
            D.metadata.error = 'Invalid or missing metadata. Please select a valid VRChat screenshot.';
            console.error('Invalid JSON input:', json);
            return;
        }
        ...
        try {
            ...
            console.error('Error parsing screenshot metadata JSON:', e, 'Input:', json);
            D.metadata = {};
            D.metadata.error = `Error parsing metadata: ${e.message}`;
            return;
        }
        ...
        // Validate extraData before parsing
        if (!extraData || extraData === 'undefined' || extraData === 'null' || typeof extraData !== 'string') {
            console.error('Invalid extraData:', extraData);
            // Continue without extra data rather than failing completely
        } else {
            try {
                const extraDataObj = JSON.parse(extraData);
                Object.assign(metadata, extraDataObj);
            } catch (e) {
                console.error('Error parsing extraData JSON:', e, 'Input:', extraData);
                // Continue without extra data rather than failing completely
            }
        }
