<template>
    <div>
        ...
        <div class="options-container">
            <span class="header">{{ t('view.settings.advanced.advanced.performance_settings.header') }}</span>
            <div class="options-container-item" style="margin-top: 15px">
                <span class="name">{{ t('view.settings.advanced.advanced.max_table_size.description') }}</span>
                <el-tooltip placement="top" :content="t('view.settings.advanced.advanced.max_table_size.tooltip')">
                    <el-input-number
                        :model-value="maxTableSize || 1000"
                        :min="100"
                        :max="10000"
                        :step="100"
                        size="small"
                        style="width: 150px; margin-left: 10px"
                        @change="handleMaxTableSizeChange" />
                </el-tooltip>
            </div>
        </div>
        <div class="options-container">
...
<script setup>
    ...
    const { showConsole, clearVRCXCache, showRegistryBackupDialog, maxTableSize } = storeToRefs(useVrcxStore());
    ...
    async function handleMaxTableSizeChange(value) {
        if (value && value >= 100 && value <= 10000) {
            maxTableSize.value = value;
            await configRepository.setString('VRCX_maxTableSize', value.toString());
            database.setMaxTableSize(value);
            feedStore.feedTableLookup();
            gameLogStore.gameLogTableLookup();
        } else if (value) {
            // If value is out of range, reset to current value
            maxTableSize.value = maxTableSize.value;
        }
    }
</script>
