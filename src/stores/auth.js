...
export const useAuthStore = defineStore('Auth', () => {
    ...
    watch(
        ...
        async ([isLoggedIn, currentUser]) => {
            ...
            if (isLoggedIn && currentUser) {
                // Wait a bit to ensure cookies are set in WebApi before saving
                await new Promise(resolve => setTimeout(resolve, 500));
                await updateStoredUser(currentUser);
                
                // Also save credentials again after a longer delay to ensure cookies are fully set
                setTimeout(async () => {
                    if (watchState.isLoggedIn && userStore.currentUser) {
                        console.log('[Auth] Periodic credential save after login');
    ...
    async function autoLoginAfterMounted() {
        // Prevent multiple auto-login attempts (e.g., from hot reload in dev mode)
        if (attemptingAutoLogin.value || watchState.isLoggedIn) {
            return;
        }
        ...
        // Get lastUserLoggedIn from config repository first
        const lastUserLoggedIn = await configRepository.getString('lastUserLoggedIn');
        if (!lastUserLoggedIn) {
            return;
        }
        
        // Set it in loginForm so it's available for other functions
        loginForm.value.lastUserLoggedIn = lastUserLoggedIn;
        
        const user = await getSavedCredentials(lastUserLoggedIn);
        if (!user) {
            return;
        }
        
        // Mark that we're attempting auto-login to prevent duplicate attempts
        ...
        if (user.cookies) {
            try {
                ...
                // Give WebApi time to process the cookies before attempting login
        ...
        authRequest
            ...
            .then(() => {
                userStore
                    ...
                    .catch((err) => {
                        ...
                        // Silently handle auto-login failures - user will need to log in manually
                        if (err?.message?.includes('Missing Credentials') || err?.message?.includes('401')) {
                            // Expected if session expired - user needs to log in again
                            return;
                        }
            ...
            .catch((err) => {
                loginForm.value.loading = false;
                attemptingAutoLogin.value = false;
                // Silently handle config fetch failures during auto-login
                if (err?.message?.includes('Missing Credentials') || err?.message?.includes('401')) {
                    // Expected if session expired
                    return;
                }
                console.error('Auto-login config error:', err);
            });
    }
    ...
    async function updateStoredUser(user) {
        if (!user || !user.id) {
            console.warn('[Auth] updateStoredUser called with invalid user:', user);
            return;
        }
        
        const savedCredentials = await getAllSavedCredentials();
        
        // Always get fresh cookies to ensure they're saved
        let cookies = '';
        try {
            cookies = await webApiService.getCookies();
            console.log('[Auth] Retrieved cookies for storage, length:', cookies?.length || 0);
            // Force save cookies to SQLite immediately
            // GetCookies() marks cookies as dirty, but we want to ensure they're persisted
            if (cookies) {
                // Small delay to ensure WebApi has processed the cookies
                await new Promise(resolve => setTimeout(resolve, 100));
            }
        } catch (e) {
            console.error('[Auth] Failed to get cookies for storage:', e);
        }
        ...
        if (credentialsToSave.value) {
            savedCredentials[user.id] = {
                ...
                cookies: cookies || '' // Save cookies on initial login too
            };
            credentialsToSave.value = null;
            console.log('[Auth] Saved new credentials for user:', user.id, 'cookies length:', cookies?.length || 0);
        } else if (typeof savedCredentials[user.id] !== 'undefined') {
            savedCredentials[user.id].user = user;
            // Always update cookies, even if empty (to clear stale cookies)
            savedCredentials[user.id].cookies = cookies || savedCredentials[user.id].cookies || '';
            console.log('[Auth] Updated existing credentials for user:', user.id, 'cookies length:', savedCredentials[user.id].cookies?.length || 0);
        } else {
            // User exists but no saved credentials - create entry with cookies
            savedCredentials[user.id] = {
                user,
                loginParams: {
                    username: '',
            ...
            console.log('[Auth] Created new credential entry for user:', user.id, 'cookies length:', cookies?.length || 0);
        }
        ...
        console.log('[Auth] Credentials saved to config repository for user:', user.id);
    }
    ...
    async function handleAutoLogin() {
        ...
        relogin(user)
            ...
            .catch((err) => {
                attemptingAutoLogin.value = false;
                // Don't show error notification for expected failures (expired session, etc.)
    ...
    async function loginComplete() {
        ...
        // Run database update after user tables are initialized
        const vrcxStore = useVrcxStore();
        await vrcxStore.updateDatabaseVersion();
