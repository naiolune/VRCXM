...
export const useGameStore = defineStore('Game', () => {
    ...
    async function checkVRChatDebugLogging() {
        ...
        try {
            ...
            // Silently handle registry file not found - this is expected if VRChat isn't installed via Steam
            if (e?.message?.includes('Registry file not found') || e?.message?.includes('not found')) {
                // Expected if VRChat isn't installed or is in a different location
                return;
            }
            console.error('VRChat registry error:', e);
        }
    }
