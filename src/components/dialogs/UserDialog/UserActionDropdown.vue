                ...
                <el-dropdown-menu>
                    ...
                    <template v-else>
                        ...
                        <el-dropdown-item
                            v-if="isFriendHidden"
                            :icon="View"
                            command="Unhide From Feed"
                            divided>
                            {{ t('dialog.user.actions.unhide_from_feed') }}
                        </el-dropdown-item>
                        <el-dropdown-item
                            v-else
                            :icon="Hide"
                            command="Hide From Feed">
                            {{ t('dialog.user.actions.hide_from_feed') }}
                        </el-dropdown-item>
                        <template v-if="userDialog.isFriend">
...
<script setup>
    ...
    import { useFeedStore, useGameStore, useLocationStore, useUserStore } from '../../../stores';
    ...
    const isFriendHidden = ref(false);

    async function checkIfHidden() {
        if (userDialog.value.ref && userDialog.value.ref.id && userDialog.value.ref.id !== currentUser.value.id) {
            isFriendHidden.value = await database.isFriendHidden(userDialog.value.ref.id);
        } else {
            isFriendHidden.value = false;
        }
    }
    ...
    // Watch for userDialog changes
    watch(
        () => userDialog.value.ref?.id,
        () => {
            checkIfHidden();
        }
    );
    ...
    async function onCommand(command) {
        if (command === 'Hide From Feed' || command === 'Unhide From Feed') {
            // Handle hide/unhide directly here
            if (command === 'Hide From Feed') {
                await database.hideFriendFromFeed(userDialog.value.ref.id);
                isFriendHidden.value = true;
            } else {
                await database.unhideFriendFromFeed(userDialog.value.ref.id);
                isFriendHidden.value = false;
            }
